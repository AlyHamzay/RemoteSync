...
const TaskDetail = () => {
  ...
  const convertBufferToBase64 = (screenshot) => {
    const buffer = screenshot?.data?.data; // Access nested buffer data
    if (buffer && Array.isArray(buffer)) {
      try {
        const uint8Array = new Uint8Array(buffer);
  
        // Convert buffer to binary string in chunks
        let binaryString = "";
  ...
  useEffect(() => {
    if (data?.task?.screenshots) {
      const screenshots = data.task.screenshots;
  
      const base64Images = screenshots.map((screenshot) => {
        if (screenshot?.data) {
          return convertBufferToBase64(screenshot);
        }
        return null;
      }).filter(Boolean);
  
      const dates = screenshots.map((screenshot) => 
  ...
  const handleButtonClick = () => {
    if (images.length > 0) {
      ...
      console.warn("No images available to display yet.");
          ...
          <>
            <div className='w-full flex flex-col md:flex-row gap-5 2xl:gap-8 bg-white shadow rounded-md px-8 py-8 overflow-y-auto'>
              <div className='w-full md:w-1/2 space-y-8'>
                ...
                <ImagesModal open={isModalOpen} onClose={() => setModalOpen(false)} images={images}  date={dates}>
                  ...
                  <div className='pl-4'>
                    ...
                    {el?.subteam?.length > 0 ? (
                      <ul className='list-disc pl-5 text-sm text-gray-700 dark:text-gray-300'>
                        {el.subteam.map((member, idx) => (
                          <li key={idx} className='mt-1'>{member.name}</li>
                        ))}
                      </ul>
                    ) : (
