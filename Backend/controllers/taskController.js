...
const updateTaskStage = asyncHandler(async (req, res) => {
  try {
    ...
    if (stage.toLowerCase() === "in progress") {
      const existingTask = await Task.findOne({ stage: "in progress" });
      if (existingTask && existingTask.id !== id) {
        return res
          .status(400)
          .json({ status: false, message: "Another task is already in progress." });
      }
    }
...
const createSubTask = asyncHandler(async (req, res) => {
  const { title, tag, date,subteam } = req.body;
  const { id } = req.params;
...
const upload_screenshot = asyncHandler(async (req, res) => {
  try {
    const { taskId } = req.params;
    const { username } = req.body; // Added to capture username
    ...
    // Adding username to the screenshot data
    task.screenshots.push({
      data: buffer,
      uploadedAt: new Date(), // Add a timestamp
      username: username // Store the username who uploaded the screenshot
    });
...
// app.use(
//   '/api/tasks/:taskId/upload_screenshot',
//   express.raw({ type: 'image/*', limit: '10mb' }), // Parse raw binary data
//   upload_screenshot
// );
